...
\title{\vspace{-14mm}\textbf{Machine Learning for Differential Equations}\\[2mm]
\large Methods, what they have solved, and what remains open\\[1mm]
\normalsize Version 1: arXiv survey}
\author{Yapi Donatien Achou\\
\small Computational Science: Physics, University of Oslo
\and
Claude Code (Anthropic)\\
\small AI assistant: literature retrieval and drafting}
\date{17 August 2026}
...
\subsection*{Across two scales}

\begin{description}[style=nextline, leftmargin=0pt]
  \item[Continuum mechanics]
  Porous media flow: Darcy flow, transport, poroelasticity.
  \item[Quantum mechanics]
  Kohn-Sham density functional theory, the self-consistent field problem.
\end{description}

\subsection*{Evaluated in two modes at each scale}

\begin{description}[style=nextline, leftmargin=0pt]
  \item[Surrogate mode]
  The model replaces the solver. Measured by accuracy on unseen conditions
  and unseen geometry.
  \item[Warm-start mode]
  The model gives the solver a good first guess to start from. Solvers work by
  improving a guess repeatedly until the answer is accurate enough, so a
  better starting guess means fewer rounds of work. The solver still decides
  when it is finished. Measured by iterations and wall-clock time saved, with
  correctness guaranteed by the solver rather than by the model.
\end{description}

\subsection*{What the axes become at the quantum scale}

The external potential plays the role that boundary conditions and
coefficients play at the continuum scale. It is set by the nuclei, which is
chemistry, and by the treatment of the cell, meaning an isolated molecule
against a periodic crystal. So unseen boundary conditions and geometry
becomes unseen chemistry and unseen cell type.

Both are stated as open in the literature surveyed below: chemistry by the
six open questions on foundation-model potentials~\citep{sixquestions}, and
transfer to periodic systems by the limitations section of the
solver-aligned initialisation work~\citep{sail}.

\subsection*{One precision}

One trained model per scale, not one model spanning both. What transfers
across scales is the architecture family, the question being asked, and the
evaluation protocol, not the weights.
...
\begin{description}[style=nextline]
  ...
  folder. On one-dimensional convection with wave speed $\beta$, the plain
  ...
  case~\citep[\S{}7]{expertguide}. For that flow around a cylinder no error is
  reported at all.
...
Cost is the other honest caveat, and GNS reports it against itself. Its own
timing appendix shows inference ranging from $51$ percent to $345$ percent of
...
Now the gap, stated as precisely as the three frontier papers allow. Each of
them closes one axis and holds the other fixed, and none closes both at once.
...
The empty cell is therefore specific: unseen geometry and unseen
...
guaranteed rather than hoped for. Nothing in this collection fills it.
...
geometries for an architecture-matched ablation (\S{}\ref{sec:structure}). And
the geometry paper's own measurement favours graph networks over the
architecture it used. Neither observation is proof, and both are the kind of
...
One caveat about how this survey uses the first two papers. They are small
...
Reading these five together produces the sharpest gap statement in this
survey, and it is a gap of omission rather than of failure.
...
\begin{description}[style=nextline]
  ...
  A 23-author discussion paper, and the strongest single source for the claim
  above. It organises the state of universal potentials around six questions
