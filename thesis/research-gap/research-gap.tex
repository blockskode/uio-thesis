...
Develop a graph neural network that learns physical laws from simulation
data, and determine whether it generalises to unseen boundary and initial
conditions and unseen domains.
...
The recurring finding is that every family works inside the distribution it
was trained on, and that they all stop at the same three obstacles: unseen
boundary conditions, unseen geometries and, at the atomistic scale, unseen
chemistries. \Cref{sec:matrix} sets this out as a table, and the row that no
paper fills, both obstacles tested together, is the thesis contribution.
...
\begin{enumerate}[itemsep=1pt, topsep=4pt]
  \item Physics-informed neural networks, which put the equation in the loss
  (\cref{sec:pinn}).
  \item Learned simulators on graphs, which learn the time-stepping map
  (\cref{sec:simulators}).
  \item Neural operators, which learn a whole family of problems at once
  (\cref{sec:operators}).
  \item Physics built into the architecture, so that the law holds by
  construction (\cref{sec:structure}).
  \item Hybrid methods, which keep the classical solver and only make it
  faster (\cref{sec:hybrid}).
  \item Reinforcement learning, which decides where to spend compute rather
  than what the answer is (\cref{sec:rl}).
  \item Atomistic and self-consistent-field methods, where the same question
  appears at the quantum scale (\cref{sec:atomistic}).
\end{enumerate}

Each family is presented in three parts: the idea in a few sentences, what it
has solved with the supporting references, and what remains open. Paper summaries
are deliberately short; the papers themselves are in \literaturelink{} for
deep reading.
...
A physics-informed neural network (PINN) uses a network $u_\theta(t, x)$ to
represent one solution of one problem, with $\theta$ the network weights, $t$
time and $x$ the space coordinate. The partial differential equation (PDE)
itself becomes the loss. Write the equation as eq.~(1)
...
\begin{description}[style=nextline]
  ...
  \item[Sanchez-Gonzalez et al., graph network-based simulators,
  GNS~\citep{sanchez2020}]
...
\begin{description}[style=nextline]
  ...
  the number of sensors needed for a target accuracy on an ordinary
  differential equation (ODE) operator, which is rare in this literature. On a diffusion-reaction problem the test error
  ...
  $0.1097$ (Table~8).
  \item[Li et al., Fourier neural operator, FNO~\citep{fno}]
...
\subsection{What remains open: what this project targets}

Before the two gap papers, the position of the three foundations should be
...
Now the gap. Three papers sit closest to it: Mousavi et al.\ and Huang
...
\begin{description}[style=nextline]
  \item[Eshaghi et al., neural operator warm starts, NOWS~\citep{nows}]
...
Machine-learned interatomic potentials (MLIPs) replace density functional
theory (DFT) inside molecular dynamics, and that community has arrived independently at
the same obstacle as the PDE community. The parallel is close enough to be
...
\subsection{The obstacle, stated by the field itself}
...
\begin{description}[style=nextline]
  \item[Eberhard et al., solver-aligned initialisation learning,
  SAIL~\citep{sail}]
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}L{4.6cm}lL{5.6cm}@{}}
    ...
    \textbf{open} & each of the three closest papers handles one and fixes the
...
Every family performs inside its training distribution, and every family
stops in the same place: at the things real applications vary, since every
patient, reservoir and molecule is a new boundary condition, geometry or
chemistry.
